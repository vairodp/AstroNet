...
def tiny_backbone(input_shape):
    ...
    output_1 = cnn_block(x, num_filters=64, kernel_size=1, strides=1, activation="mish")

    output_2 = csp_block(output_1, filters=128, num_blocks=2)

    output_3 = csp_block(output_2, filters=256, num_blocks=8)

    return tf.keras.Model(inputs, [output_1, output_2, output_3], name="TinyYOLOBackbone")
...
class TinyYOLOv4(tf.keras.Model):
    def __init__(self, input_shape, num_classes,
        ...
        backbone = tiny_backbone(input_shape)
        ...
        self.model = tf.keras.Model(inputs=inputs, outputs=upper_features, name="TinyYOLOv4")
