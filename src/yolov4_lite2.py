...
def cspdarknet53_tiny(input_shape):
    ...
    output_2 = cnn_block(input_data, num_filters=256, kernel_size=3, strides=1)
...
class YOLOv4Lite(tf.keras.Model):
    def __init__(self, input_shape, num_classes,
        ...
        out_lbbox = scale_prediction(route, num_anchors_stage=len(anchors[0]), num_classes=NUM_CLASSES)

        output_2 = cnn_block(output_2, num_filters=64, kernel_size=1, strides=1)
        output_2 = upsample(output_2)
        output_2 = tf.concat([output_2, output_1], axis=-1)

        route = cnn_block(output_2, num_filters=128, kernel_size=3, strides=1)
